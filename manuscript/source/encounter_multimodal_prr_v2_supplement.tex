...
\begin{equation}
 ...
 V_w=\sum_{j=1}^m w_jV_j,\qquad K_{B,w}=BV_w,
...
\begin{equation}
 \sup_{t\in I}|r_*(t)|_{\rm mi}\le a-\eta_*
 \quad\text{for some }\eta_*>0.
...
conservative.  This fixed-index plug-in scale does not account for the
data-driven selection of the peak and contour-base bins; the five-sigma
rule is an operational gate, not a calibrated post-selection significance
level.  A peak-only convention
...
counts.  The mean-field crossings \(B_{\rm op}^{\rm mf}\), the 96-cell
verdict comparison, the production peak-time comparison, and the contact
factors \(c(\tau;\varepsilon)\) quoted for the mean-field hazard--survival
law of the main text are computed from the free-clock evaluation by
\texttt{exact\_m\_prr\_mean\_field\_boundary.py}; the numerical
conventions, grid checks, and outputs are stored in
\texttt{mean\_field\_boundary.json} in the reproduction archive.
...
\(t=2.49\) with \(4.8\sigma_{\rm prom}\) prominence and only \(2.7\%\) of the maximum
...
event mass & min.\ prom.\ [\(\sigma_{\rm prom}\)]\\
\hline
...
\(4.2\)--\(4.4\sigma_{\rm prom}\) at \(5.3\)--\(7.5\%\) relative prominence with
\(10^{6}\) walkers, and \(3.4\sigma_{\rm prom}\) at \(3.5\%\) in the
...
covariance-aware standard deviations, so the \(5\times10^{5}\)-walker
\(\Delta t/2\) value is a single low outlier among seven runs.  Three
additional independent-stream runs of this cell at
\(\Delta t=5\times10^{-4}\) (seeds \(20260814\)--\(20260816\),
\(10^{6}\) walkers each, matching the walker count of the three
\(\Delta t=10^{-3}\) seed repeats; the streams at the two resolutions are
independent, not paired common random numbers) all retain three classified
modes under the covariance-aware rule, with the late candidate at
\(6.9\)--\(8.6\%\) of the global maximum and \(9.8\)--\(12.3\)
covariance-aware standard deviations, against \(6.3\)--\(6.6\%\) and
\(9.0\)--\(9.4\) for the matched-walker \(\Delta t=10^{-3}\) seeds.
...
contribution near this classifier boundary.
...
\((\varepsilon,B)\) cells (blue: both target modes retained; the printed
number is the classified count), and the semi-analytic free clock retains both
...
replicas; the per-replica attribution of failures to window-end displacement
versus spacing collapse is given in the main text (Sec.~IV\,F).  Right:
...
reaction events per
walker inside the window and \(\Delta t\)-halving maximum standardized bin
difference \(|z_{\rm bin}|=1.67\) (Sec.~\ref{sec:supp-numerics-method}).
Dashed curve: free-exposure prediction \(BG(t)\) with the \(d=3\) contact
factor; dash-dotted curve: mean-field hazard--survival law
\(f_1=BG\,\e^{-B\Lambda}\) [Eq.~(24) of the main text, Sec.~IV\,E], shown
for comparison only; it lies above the smoothed histogram at the late peak.
(Plotting code revised with Codex assistance; numerical content
author-verified.)
\label{fig:supp-d3}}
\end{figure}
...
Eqs.~\eqref{eq:exactmfull-log-slope}--\eqref{eq:exactmfull-log-slope-derivative}, and the crossover ratio and nonadjacent
...
\texttt{admit}, or added axiom.  Of its 174 theorem and lemma declarations,
the 138 theorems audited in its consolidated \texttt{\#print axioms}
report (\texttt{consolidated\_axioms.txt}) report
exactly the standard foundation \([\texttt{propext},
\texttt{Classical.choice}, \texttt{Quot.sound}]\); the remaining
declarations are auxiliary lemmas and a smoke test and are not listed
...
statement-fidelity of every audited theorem were additionally audited
